...
namespace PHPdbug
{
    class Debug
    {
        public static function dump($var, $exit = false, $forceConsole = false, $save = false, $backtrace = false)
        {
            $filename = false;
            ...
            $type = gettype($var);
            if(php_sapi_name() == 'cli' || $_SERVER['HTTP_ACCEPT'] == 'application/json' || $forceConsole){
                echo "\n\n";
                if($filename){
                    echo 'File: ' . $filename . "\n";
                }
